    ...
    for categ, eps in ep_dict.items():
        for ep in eps:
            if pd.notna(ep) and type(ep) == str:
                result = jumanpp.analysis(ep).mrph_list()
                all_words.extend([a.midasi for a in result if a.hinsi in pos and a.midasi not in Prohibited and a.bunrui not in STOPPOS_JP])
                word_dict[categ].extend([a.midasi for a in result if a.hinsi in pos and a.midasi not in Prohibited and a.bunrui not in STOPPOS_JP])
